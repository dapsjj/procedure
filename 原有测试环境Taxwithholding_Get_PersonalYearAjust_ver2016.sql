...
/****** Object:  StoredProcedure [dbo].[Taxwithholding_Get_PersonalYearAjust_ver2016]    Script Date: 2019/04/18 17:00:12 ******/
SET ANSI_NULLS ON
GO
...
--exec Taxwithholding_Get_PersonalYearAjust_ver2016 10076815,2,10076815,2 
--***********************************************************************  
...
SELECT @AdjustYear = isnull(min(AdjustYear ),2015)  
...
BEGIN
    ...
    select
        明細ID  ObjID
        ,コード EmployeeManageID
        ,case when  配偶者区分 =1 then  'A'
            when DATEDIFF(DAY,DATEADD(year,16, 生年月日),CONVERT(DATE, convert(varchar(4),@AdjustYear) + '/01/02'))<=0 then 'M'
    ...
    insert into #AObj
    select EmployeeManageID,Name,Name_syllabary,Inmate
    from #maintainObj
    where ObjKind='A'
    ...
    on ins.EmployeeManageID=a.EmployeeManageID
    ...
    BEGIN
        ...
        if(@indext=1)
            insert into #BObj
            values(@employeeManageID,@name,@name_syllabary,@inmate,'','',0,'','',0,'','',0)
        else if(@indext=2)
            update  #BObj set Name2 =@name,Name_syllabary2=@name_syllabary,Inmate2 =@inmate  
        else if(@indext=3)
            update  #BObj set Name3 =@name,Name_syllabary3=@name_syllabary,Inmate3 =@inmate  
        else if(@indext=4)
            update  #BObj set Name4 =@name,Name_syllabary4=@name_syllabary,Inmate4 =@inmate  
        else if(@indext>4)
        BEGIN
            set @strIndex=@strIndex+1
            set @str =@str+'('+CONVERT(varchar,@strIndex)+')'+' '+@name+@inmateName+' '
        END
        set @count=@count-1
        set @indext=@indext+1
    END
    ...
    BEGIN
        ...
        if(@indext=1)
            ...
            update  #MObj set Name2 =@name,Name_syllabary2=@name_syllabary,Inmate2 =@inmate  
       else if(@indext=3)
            update  #MObj set Name3 =@name,Name_syllabary3=@name_syllabary,Inmate3 =@inmate  
       else if(@indext=4)
            update  #MObj set Name4 =@name,Name_syllabary4=@name_syllabary,Inmate4 =@inmate  
       else if(@indext>4)
       BEGIN
            if(@inmate = 0) set @inmateName='' else set @inmateName='（非居住者）'
            set @strIndex=@strIndex+1
            set @str =@str+'('+CONVERT(varchar,@strIndex)+')'+' '+@name+'（年少）'+@inmateName+' '
       END
        set @count=@count-1
        set @indext=@indext+1
...
SET @SQL2 = ',''給料・賞与'' AS ''給料・賞与''  
     ,convert(int,OYAA.合計課税総額)/1000000 AS ''合計課税総額百万''
     ,convert(int,OYAA.合計課税総額)/1000%1000 AS ''合計課税総額千'' 
     ,convert(int,OYAA.合計課税総額)%1000 AS ''合計課税総額1'' 
     ,OYAA.合計課税総額 AS ''合計課税総額''
     ,convert(int,OYAA.所得控除後金額)/1000000 AS ''所得控除後金額百万''
     ,convert(int,OYAA.所得控除後金額)/1000%1000 AS ''所得控除後金額千''
     ,convert(int,OYAA.所得控除後金額)%1000 AS ''所得控除後金額1''
     ,OYAA.所得控除後金額 AS ''所得控除後金額''
     ,convert(int,OYAA.所得控除額合計)/1000000 AS ''所得控除額合計百万''
     ,convert(int,OYAA.所得控除額合計)/1000%1000 AS ''所得控除額合計千''
     ,convert(int,OYAA.所得控除額合計)%1000 AS ''所得控除額合計1''
     ,OYAA.所得控除額合計 AS ''所得控除額合計''
     ,convert(int,(CASE WHEN ( (OYAA.年度 = 2010 and OYAA.税額表区分 = 2) or (OYAA.年度 >= 2011 and OYAA.対象者区分 = 0)  ) 
            THEN OYAA.合計徴収済税額 ELSE OYAA.差引年税額 END))/1000000 AS ''差引年税額百万''  
     ,convert(int,(CASE WHEN ( (OYAA.年度 = 2010 and OYAA.税額表区分 = 2) or (OYAA.年度 >= 2011 and OYAA.対象者区分 = 0)  ) 
            THEN OYAA.合計徴収済税額 ELSE OYAA.差引年税額 END))/1000%1000 AS ''差引年税額千'' 
     ,convert(int,(CASE WHEN ( (OYAA.年度 = 2010 and OYAA.税額表区分 = 2) or (OYAA.年度 >= 2011 and OYAA.対象者区分 = 0)  ) 
            THEN OYAA.合計徴収済税額 ELSE OYAA.差引年税額 END))%1000 AS ''差引年税額1''
...
SET @SQL3 = ',CASE WHEN OME.対象者区分=1 AND OME.配偶者控除区分 = 1  
 ...
 ,CASE WHEN OME.対象者区分=1 then convert(int ,OYAA.配特別控除額)/1000 else 0 end  AS ''配特別控除額千''  
 ,CASE WHEN OME.対象者区分=1 then convert(int ,OYAA.配特別控除額)%1000 else 0 end  AS ''配特別控除額1''  
 ...
 ,convert(int,OYAA.合計社保控除額)/1000 AS ''合計社保控除額千''
 ,convert(int,OYAA.合計社保控除額)%1000 AS ''合計社保控除額1''
 ,OYAA.合計社保控除額 AS ''合計社保控除額''
 ,convert(int,OYAA.合計生保控除)/1000 AS ''合計生保控除千''--生命保険料の控除額
 ,convert(int,OYAA.合計生保控除)%1000 AS ''合計生保控除1''--生命保険料の控除額
 ,OYAA.合計生保控除 AS ''合計生保控除''--生命保険料の控除額  
--20101022  
-- ,OYAA.EarthquakeInsDeduct AS ''地震険料の控除額''  
 ,convert(int,OYAA.合計地震控除)/1000 AS ''合計地震控除千''--地震険料の控除額
 ,convert(int,OYAA.合計地震控除)%1000 AS ''合計地震控除1''--地震険料の控除額
 ,OYAA.合計地震控除 AS ''合計地震控除''--地震険料の控除額
 ,convert(int,OYAA.住宅特別控除)/1000   AS ''住宅借入金等特別控除の額千''
 ,convert(int,OYAA.住宅特別控除)%1000  AS ''住宅借入金等特別控除の額1''
...
SET @SQL7 = ', OYAA.年調給与額     AS ''合計総支給額''  
                      ...
                      , convert(int,OYAA.小規模企業共済)/1000       AS ''小規模企業共済千''
                      , convert(int,OYAA.小規模企業共済)%1000       AS ''小規模企業共済1''
...
SET @SQL13  =  ',OYAA.特別控除適用数
     ,OYAA.住宅控除申告  AS 特別控除可能額
     ,YEAR(OYAA.居住開始年月日1回目)  as 居住開始年1
     ,MONTH(OYAA.居住開始年月日1回目) as 居住開始月1
     ,DAY(OYAA.居住開始年月日1回目)  as 居住開始日1
     ,YEAR(OYAA.居住開始年月日2回目) as 居住開始年2
     ...
     ,OYAA.年末残高1回目
     ,OYAA.年末残高2回目
     ,ISNULL((select Name  from #AObj),'''') AS 扶養名A
     ,ISNULL((select Name_syllabary from #AObj),'''')  扶養名カナA
     ,ISNULL((select CASE when ISNULL(Inmate,0)= 0 then '''' else ''〇'' end from #AObj),'''') 扶養区分A
     ,ISNULL((select  Name1  from #BObj),'''') 扶養名B1
     ,ISNULL((select Name_syllabary1 from #BObj),'''')  扶養名カナB1
     ,ISNULL((select CASE when ISNULL(Inmate1,0)= 0 then '''' else ''〇'' end from #BObj),'''') 扶養区分B1
     ,ISNULL((select Name2  from #BObj),'''') 扶養名B2
     ,ISNULL((select Name_syllabary2 from #BObj),'''')  扶養名カナB2
     ,ISNULL((select CASE when ISNULL(Inmate2,0)= 0 then '''' else ''〇'' end from #BObj),'''') 扶養区分B2
     ,ISNULL((select Name3  from #BObj),'''') 扶養名B3
     ,ISNULL((select Name_syllabary3 from #BObj),'''')  扶養名カナB3
     ,ISNULL((select CASE when ISNULL(Inmate3,0)= 0 then '''' else ''〇'' end from #BObj),'''') 扶養区分B3
     ,ISNULL((select Name4 from #BObj),'''') 扶養名B4
     ,ISNULL((select Name_syllabary4 from #BObj),'''')  扶養名カナB4
     ,ISNULL((select CASE when ISNULL(Inmate4,0)= 0 then '''' else ''〇'' end from #BObj),'''') 扶養区分B4
     ,ISNULL((select Name1 from #MObj),'''') 扶養名M1
     ,ISNULL((select Name_syllabary1 from #MObj),'''')  扶養名カナM1
     ,ISNULL((select CASE when ISNULL(Inmate1,0)= 0 then '''' else ''〇'' end from #MObj),'''') 扶養区分M1
     ,ISNULL((select Name2 from #MObj),'''') 扶養名M2
     ,ISNULL((select Name_syllabary2 from #MObj),'''')  扶養名カナM2
     ,ISNULL((select CASE when ISNULL(Inmate2,0)= 0 then '''' else ''〇'' end from #MObj),'''') 扶養区分M2
     ,ISNULL((select Name3 from #MObj),'''') 扶養名M3
     ,ISNULL((select Name_syllabary3 from #MObj),'''')  扶養名カナM3
     ,ISNULL((select CASE when ISNULL(Inmate3,0)= 0 then '''' else ''〇'' end from #MObj),'''') 扶養区分M3
     ,ISNULL((select Name4  from #MObj),'''') 扶養名M4
     ,ISNULL((select Name_syllabary4 from #MObj),'''')  扶養名カナM4
     ,ISNULL((select CASE when ISNULL(Inmate4,0)= 0 then '''' else ''〇'' end from #MObj),'''') 扶養区分M4
     ,'+convert(varchar(3),@inmateCount)+'AS 非居住者親族数'
