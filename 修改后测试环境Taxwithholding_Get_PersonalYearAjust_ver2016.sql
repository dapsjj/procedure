...
/****** Object:  StoredProcedure [dbo].[Taxwithholding_Get_PersonalYearAjust_ver2016]    Script Date: 2019/04/19 14:10:39 ******/
SET ANSI_NULLS ON
GO
...
ALTER  PROC [dbo].[Taxwithholding_Get_PersonalYearAjust_ver2016]  
...
SET @SQL5 = ',CASE WHEN DATEDIFF(DAY, (CONVERT(VARCHAR(4),(YEAR('''+convert(varchar(4),@AdjustYear)+'/12/01'') - 19))+ ''/01/03''),OME.生年月日) > 0   
 ...
 ,CASE WHEN SUBSTRING(( dbo.FN_Year_GetYear(OME.生年月日)),1,1) = ''平'' --★  
  ...
  ELSE ''''  
  END   AS ''受給者生年平''   
 ,CASE WHEN SUBSTRING(( dbo.FN_Year_GetYear(OME.生年月日)),1,1) = ''令'' --★  
  THEN ''○''  
  ELSE ''''  
  END   AS ''受給者生年令''  
